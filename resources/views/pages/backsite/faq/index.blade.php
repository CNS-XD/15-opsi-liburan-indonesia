...
@section('title', 'Faq')
@section('activeMenuFaq', 'open active')
@section('activeSubMenuFaq', 'open active')

{{-- Breadcrumb --}}
@section('breadcrumb1', 'Information')
@section('breadcrumb2', 'Faq')

{{-- Button Pojok Kanan --}}
@section('buttonRight')
<a href="{{ route('backsite.faq.create') }}" class="btn btn-success btn-sm btn-glow round">
    <i class="fa fa-plus"></i> Add
</a>
<a href="{{ route('frontsite.faq.index') }}" class="btn btn-info btn-sm btn-glow round" target="_blank">
...
<script>
    ...
    const datatable = () => {
        ...
        $('#dataTable').DataTable({
            ...
            "ajax": {
                url: '{{ route('backsite.faq.datatable') }}',
            },
            columns: [
                {
                    data: 'DT_RowIndex',
    ...
    function setShow(id) {
        ...
        let url = `{{ route('backsite.faq.set-show', ':id') }}`;
    ...
    // Delete
    const deleteRoute = "{{ route('backsite.faq.destroy', ':id') }}";
